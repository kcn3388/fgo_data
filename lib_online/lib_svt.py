...
def lib_svt(svt_data: dict) -> dict:
    ...
    get_pickup(
        svt, BeautifulSoup(raw_html, 'html.parser'), f'https://fgo.wiki/w/{svt_data["name_link"]}'
    )
...
def get_card_url(svt: dict, raw_html: str, raw_data: str):
    ...
    for card_index in range(len(all_files)):
        try:
            ...
            print(f"unmatch len of url and card for {svt['id']}")
            print(f"url: {len(cards_url)}; card: {len(cards)}")
            svt["error"].append(
...
def get_info(svt: dict, raw_data: str):
    ...
    for info_index in range(len(all_info)):
        try:
            ...
            print(f"unmatch len of info for {svt['id']}")
            print(
...
def get_ultimate(svt: dict, raw_data: str):
    ...
    if not raw_ultimates:
        if svt["id"] == "152" or svt["id"] == "411" or svt["id"] == "412":
            svt["宝具信息"] = ultimates
            return
...
def get_skills(svt: dict, raw_data: str):
    ...
    for ns_index in range(len(all_normal_skills)):
        ...
        try:
            ...
            print(f"unmatch len of skill and skill name for {svt['id']}")
            print(f"name: {len(all_normal_names)}; skill: {len(all_normal_skills)}")
...
def get_tabber_skills(svt: dict, all_raw_skills: str, skills: dict):
    ...
    try:
        ...
        print(f"unmatch len of multi skill and skill name for {svt['id']}")
        print(
...
async def get_pickup(svt: dict, origin_soup: BeautifulSoup, url: str):
    new_url = f"{url}/未来Pick_Up情况"
    try:
        soup = BeautifulSoup(requests.get(new_url, timeout=20, headers=headers).content, 'html.parser')
    ...
    for each_td in all_tds:
        ...
        try:
            ...
            new_url = f"https://fgo.wiki/index.php?title={new_href}&action=edit"
            raw_time = BeautifulSoup(
                requests.get(new_url, timeout=20, headers=headers).content, 'html.parser'
            ).find("textarea").text
            try:
                ...
                img = re.search(
                    rf"https://media.fgo.wiki(/thumb)?/./../{quote(img_file)}", str(soup)
                ).group(0).replace("/thumb", "")
